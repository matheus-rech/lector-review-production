...
function PDFViewerContent({
  ...
  // Use Lector hooks with correct API
  ...
  }, [debouncedSearchTerm, search, onSearchResultsChange, onUpdateSearchHighlights, onSearchError]);
  ...
  // Handle text selection - store pending selection using getDimension()
  useEffect(() => {
  ...
  // Convert highlights to Lector format (ColoredHighlight with rectangles, text, uuid)
    ...
    <div style={{ position: "relative", width: "100%", height: "100%" }}>
      <Pages>
        <Page>
          <CanvasLayer />
          <TextLayer />
          <ColoredHighlightLayer onHighlight={(highlight) => console.log('Highlight clicked:', highlight)} />
        </Page>
      </Pages>
...
export default function App() {
  ...
  /** Page Form Data with Undo/Redo */
  const {
    state: pageForm,
    setState: setPageForm,
    undo,
    redo,
    canUndo,
    canRedo,
  ...
  const shortcuts: KeyboardShortcut[] = [
    { ...commonShortcuts.undo, callback: () => { if (canUndo) { undo(); toast.info('Undo'); } } },
    { ...commonShortcuts.redo, callback: () => { if (canRedo) { redo(); toast.info('Redo'); } } },
    { ...commonShortcuts.search, callback: () => searchInputRef.current?.focus() },
    { ...commonShortcuts.nextPage, callback: () => jumpToPage(currentPage + 1) },
    { ...commonShortcuts.prevPage, callback: () => jumpToPage(currentPage - 1) },
    { ...commonShortcuts.exportJSON, callback: exportJSON },
    { ...commonShortcuts.exportCSV, callback: exportCSV },
    { ...commonShortcuts.newProject, callback: () => setAddProjectModal(true) },
    { ...commonShortcuts.help, callback: () => setHelpModal(true) },
  ];
  ...
  const addProject = (name: string) => {
    ...
    } else if (projects.indexOf(name) !== -1) {
  ...
  const relabelHighlight = (id: string, newLabel: string) => {
    const h = highlights.filter((x) => x.id === id)[0];
    if (!h) return;
    if (newLabel) {
      setHighlights((prev) =>
  ...
  function exportJSON() {
    try {
      setIsLoading(true);
      setLoadingMessage("Exporting JSON...");
  ...
  function exportCSV() {
    try {
      ...
      const formEntries = Object.keys(pageForm).map(key => [key, pageForm[key]]);
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      {/* Loading overlay */}
      {isLoading && <Loading message={loadingMessage} overlay size="large" />}
      ...
      <InputModal
        ...
        defaultValue={
          relabelModal.highlightId
            ? (highlights.filter((h) => h.id === relabelModal.highlightId)[0]?.label || "")
            : ""
        }
        onConfirm={(newLabel) => {
      ...
      <aside className="w-64 border-r p-3 space-y-4 bg-white overflow-y-auto">
        ...
        <div className="flex gap-2">
          <button
            ...
            onClick={exportJSON}
            title="Export JSON (Ctrl+Shift+E)"
          >
            Export JSON
          </button>
          <button
